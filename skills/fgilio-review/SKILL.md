...
1. **Mode** — the first word that names a mode. `report` (default) presents findings and changes nothing; `address` implements the fixes; `comment` posts findings as review comments on a pull request. Accept synonyms: `fix` → address, `pr` / `comment-pr` → comment. A word that names no mode never selects one — the default stands.
